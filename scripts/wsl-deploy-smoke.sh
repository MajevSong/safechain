#!/usr/bin/env bash
# Deploy the SafeChain chaincode to the competency channel and run a live
# on-chain smoke test (register -> verify -> authorize -> accident/premium).
# Run from WSL: bash /mnt/d/Dev/simulation_blockchain/scripts/wsl-deploy-smoke.sh
set -e

TN="$HOME/safechain/fabric-samples/test-network"
SRC="/mnt/d/Dev/simulation_blockchain/chaincode"
CC="$HOME/safechain/chaincode"        # WSL-native copy (no Windows node_modules)

echo "=== sync chaincode to WSL (exclude node_modules/test) ==="
mkdir -p "$CC"
...
echo "=== deploy chaincode 'safechain' to competency channel ==="
./network.sh deployCC -c competency -ccn safechain -ccp "$CC" -ccl javascript
...
query() {
  peer chaincode query -C competency -n safechain -c "$1"
}
...
echo "--- 7) GetPremium -> factor 1.19 ---"
query '{"function":"AccidentPremiumContract:GetPremium","Args":["default"]}'
